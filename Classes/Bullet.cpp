...
bool Bullet::init(Vec2 position, float speed, int dir)
{
	...
	//类型为普通子弹
	m_texture = Director::getInstance()->getTextureCache()->addImage("image/tank/bullet.png");
	switch (dir)
	{
	case BULLET_UP:
		m_sprite = Sprite::createWithTexture(m_texture, Rect(  0, 0, 8, 8));//旋转
		break;
	case BULLET_DOWN:
...
void Bullet::Blast()
{
	...
	setLife(0);
	auto explode = Sprite::create("image/tank/explode1.png");
