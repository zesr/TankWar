...
bool Tank::init(int ID, float x, float y, int dir, int kind)
{
	...
	if (kind == 1)
	{
		m_texture = Director::getInstance()->getTextureCache()->addImage("image/tank/player1.png");
	}
	if (kind == 2)
	{
		m_texture = Director::getInstance()->getTextureCache()->addImage("image/tank/player2.png");
	}
...
void Tank::Fire(float time)
{
	if (time!=-1&&time - lastTime1 <= 1)
		return;
	lastTime1 = time;
	//炮击音效
	...
	auto bullet = Bullet::create(position,5, this->getDirection());
...
void Tank::Blast()
{
	...
	this->setLife(0);
	auto explode = Sprite::create("image/tank/explode2.png");
